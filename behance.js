var Behance = (function () {
    function Behance() {
        this.loadCoverProjects();
    }
    Behance.prototype.loadCoverProjects = function (n) {
        if (n === void 0) { n = 0; }
        Behance.num += n;
    ...
    Behance.prototype.showFullProject = function (id) {
    ...
    Behance.prototype.renderViewCover = function () {
        ...
        for (var i = 0; i < lg; i++) {
            ...
            for (var x in projetos[i].fields)
                ul += '<i class="fields">' + projetos[i].fields[x] + ', </i>';
            ul += '</div>';
        ...
        if (Behance.num > 1)
            ul += '<button class="btn-return" onclick="be.loadCoverProjects(-1)">Retornar</button>';
        jQuery('#projects').html(ul);
    };
    Behance.prototype.renderViewProject = function (id) {
